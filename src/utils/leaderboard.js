...
async function buildLeaderboardEmbed() {
  ...
  if (inServiceList.length === 0) {
    ...
    inServiceList.forEach((agent, i) => {
      ...
      const lien = agent.lienAppel ? `\n　　📋 [Appel intervention](${agent.lienAppel})` : '';
  ...
  if (allRanked.length === 0) {
    totalField = '*Aucune heure enregistree.*';
  } else {
  ...
  return new EmbedBuilder()
    .setTitle('🔐 Departement de Securite — Tableau de Service')
    .setDescription('> *Fondation SCP — Unite de Securite Operationnelle*\n\u200b')
    .setColor(0x1a1a2e)
    ...
    .setFooter({ text: 'Fondation SCP • Departement de Securite • Mis a jour' })
    .setTimestamp();
}
