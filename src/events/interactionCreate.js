...
async function updateLeaderboardMessage(channel) {
  try {
    ...
    await leaderboardMsg.edit({ embeds: [embed], components: [buttons] });
  } catch (err) {
    console.error('Impossible de mettre a jour le leaderboard:', err);
  }
}
...
module.exports = {
  ...
  async execute(interaction, client) {
      ...
      if (interaction.commandName === 'setup-service') {
        if (SERVICE_CHANNEL_ID && interaction.channelId !== SERVICE_CHANNEL_ID) {
          return interaction.reply({
            content: `🚫 Cette commande ne peut etre utilisee que dans <#${SERVICE_CHANNEL_ID}>.`,
            ephemeral: true
          });
        }
      }
    ...
    if (interaction.isModalSubmit() && interaction.customId === 'modal_prise_service') {
      ...
      await setAgent(userId, { inService: true, startTime: now, username: interaction.user.username, lienAppel });
      ...
      // Embed confirmation (visible uniquement par l'agent)
      const confirmEmbed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: 'Total accumule', value: `\`${formatDuration(agent.totalMs)}\``, inline: true },
      ...
      const logEmbed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: 'Heure', value: `<t:${Math.floor(now / 1000)}:T>`, inline: true },
          { name: 'Total accumule', value: `\`${formatDuration(agent.totalMs)}\``, inline: true },
          { name: 'Lien appel intervention', value: lienAppel, inline: false }
        )
        .setColor(0x00ff88)
        .setThumbnail(interaction.user.displayAvatarURL())
        .setFooter({ text: 'Fondation SCP • Logs de Service' })
        .setTimestamp();

      await sendLog(client, logEmbed);
      await updateLeaderboardMessage(interaction.channel);
      return;
    }
    ...
    if (interaction.customId === 'prise_service') {
      if (agent.inService) {
        return interaction.reply({
          content: `Tu es **deja en service** ! Utilise le bouton **Fin de Service** pour terminer.`,
          ephemeral: true
        });
      }
    ...
    if (interaction.customId === 'fin_service') {
      if (!agent.inService) {
        return interaction.reply({
          content: `Tu n'es **pas en service** actuellement !`,
          ephemeral: true
        });
      }
      ...
      await setAgent(userId, { inService: false, startTime: null, totalMs: newTotal, username: interaction.user.username, lienAppel: null });

      if (IN_SERVICE_ROLE_ID) {
        try { await interaction.member.roles.remove(IN_SERVICE_ROLE_ID); }
        catch (e) { console.warn('Role remove error:', e.message); }
      }
      ...
      const confirmEmbed = new EmbedBuilder()
        ...
        .addFields(
          { name: 'Duree de la session', value: `\`${formatDuration(sessionMs)}\``, inline: true },
